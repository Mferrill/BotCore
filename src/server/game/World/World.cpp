...
void World::LoadConfigSettings(bool reload)
{
	...
	rate_values[RATE_XP_KILL_ELITE]     = sConfig.GetFloatDefault("Rate.XP.Kill.Elite", 2.0f);
...
void World::ShutdownMsg(bool show, Player* player)
{
        ...
        (m_ShutdownTimer>12*HOUR   && (m_ShutdownTimer % (12*HOUR)) == 0))
    {
		...
		sLog.outDebug("Server is %s in %s",(m_ShutdownMask & SHUTDOWN_MASK_RESTART ? "restart" : "shuttingdown"),str.c_str());
    }
}
...
void World::ShutdownCancel()
{
    ...
    sLog.outDebug("Server %s cancelled.",(m_ShutdownMask & SHUTDOWN_MASK_RESTART ? "restart" : "shuttingdown"));
}
