...
void Script::RegisterSelf()
{
    ...
    if (id)
    {
        ...
        else
        {
            ...
            sLog.outError("ScriptName: '%s' already assigned with the same ScriptName, Check Scriptloader you have it loading more than once. ", Name.c_str());
            delete this;
        }
    }
    else
    {
        if (Name.find("example") == std::string::npos)
			sLog.outErrorDb("Do I smell a BrainFart?  script named %s Is not attached to a gameobject item or npc Please check your DB.",(this)->Name.c_str());
        delete this;
    }
}
