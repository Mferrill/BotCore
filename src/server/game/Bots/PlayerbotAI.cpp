...
PlayerbotAI::PlayerbotAI(Player *master, Player *bot) : m_master(master), m_bot(bot),
...
m_spellIdCommand(0), m_targetGuidCommand(0), m_classAI(NULL), isLooting(false), m_TimeRessurect(0),
m_FeastSpamTimer(0)
{
    ...
    {
        case CLASS_PRIEST:
            m_classAI = (PlayerbotClassAI *)new PlayerbotPriestAI(master, bot, this);
            break;
        case CLASS_WARRIOR:
            m_classAI = (PlayerbotClassAI *)new PlayerbotWarriorAI(master, bot, this);
            break;
        case CLASS_SHAMAN:
            m_classAI = (PlayerbotClassAI *)new PlayerbotShamanAI(master, bot, this);
            break;
        case CLASS_ROGUE:
            m_classAI = (PlayerbotClassAI *)new PlayerbotRogueAI(master, bot, this);
            break;
        case CLASS_PALADIN:
            m_classAI = (PlayerbotClassAI *)new PlayerbotPaladinAI(master, bot, this);
            break;
        case CLASS_MAGE:
            m_classAI = (PlayerbotClassAI *)new PlayerbotMageAI(master, bot, this);
            break;
        case CLASS_DRUID:
            m_classAI = (PlayerbotClassAI *)new PlayerbotDruidAI(master, bot, this);
            break;
        case CLASS_WARLOCK:
            m_classAI = (PlayerbotClassAI *)new PlayerbotWarlockAI(master, bot, this);
            break;
        case CLASS_HUNTER:
            m_classAI = (PlayerbotClassAI *)new PlayerbotHunterAI(master, bot, this);
            break;
        case CLASS_DEATH_KNIGHT:
            m_classAI = (PlayerbotClassAI *)new PlayerbotDeathKnightAI(master, bot, this);
            break;
    }

    //load config variables
    m_followDistanceMin = sConfig.GetFloatDefault("Bot.FollowDistanceMin", 0.5f);
    m_followDistanceMax = sConfig.GetFloatDefault("Bot.FollowDistanceMax", 1.5f);
...
//sLog.outError ("gameobject type = %u", obj->GetGoType());
        ...
        case CMSG_LIST_INVENTORY:
        {
			...
			Unit *const pNpc = masterSession.GetPlayer()->GetSelectedUnit();
			if (!pNpc || (!pNpc->isVendor() && !pNpc->isInnkeeper())) return;
                ...
                it != masterSession.GetPlayerBotsEnd(); ++it)
            {
                ...
                else
                {
                    uint32 TotalCost = 0;
                    uint32 TotalSold = 0;
					uint32 canSellCount = 0;
                    std::ostringstream report;
                    std::ostringstream canSell;
                    canSell << "The following items are not trash and can be sold: \n";
                    // list out items in main backpack
                    for (uint8 slot = INVENTORY_SLOT_ITEM_START; slot < INVENTORY_SLOT_ITEM_END; slot++)
                    {
					...
					// if the bot sold some junk items sold, let the mater know
                    if (TotalSold > 0) {
				    ...
				    // if the bot can sell some items, let the master know which
					if (canSellCount > 0)
						bot->GetPlayerbotAI()->TellMaster(canSell.str());
					else
						bot->GetPlayerbotAI()->TellMaster("I don't have anything to sell at the moment.");
                }
            }
...
void PlayerbotAI::HandleBotOutgoingPacket(const WorldPacket &packet)
{
    switch(packet.GetOpcode())
    {
        ...
        case SMSG_SPELL_GO:
        {
            ...
            uint64 casterGuid = extractGuid(p);
            //if(m_bot && casterGuid != m_bot->GetGUID()) return;

            uint32 spellId; p >> spellId;
            uint16 castFlags; p >> castFlags;
            uint32 msTime; p >> msTime;
            uint8 numHit; p >> numHit;
            /*if(GetCurrentSpell() && m_CurrentlyCastingSpellId == spellId)
            {
        ...
        case SMSG_TEXT_EMOTE:
        {
            ...
            switch(text_emote)
            {
                case TEXTEMOTE_BOW:
                {
                    ...
                    if(bot && bot->GetGUID()==m_bot->GetGUID() && bot->GetPlayerbotAI()->GetClassAI())
                    {
...
void PlayerbotAI::Feast()
{
    ...
    if(m_bot->getPowerType() == POWER_MANA && currentTime > m_TimeDoneDrinking && ((static_cast<float>(m_bot->GetPower(POWER_MANA)) / m_bot->GetMaxPower(POWER_MANA)) < 0.8))
    {
        ...
        if(pItem && !m_bot->HasSpellCooldown(pItem->GetSpell()))
        {
            UseItem(*pItem);
            ...
            if (m_FeastSpamTimer > 0) --m_FeastSpamTimer;
				...
				m_bot->GetPlayerbotAI()->TellMaster("I need water."); 
...
void PlayerbotAI::UpdateAI(const uint32 p_time)
{
	...
	if (!m_playerBotsFly && m_master->isInFlight())
    {
    ...
    else if (!m_bot->isInCombat() && m_classAI->FindMainTankInRaid(m_bot) && m_classAI->FindMainTankInRaid(m_bot)->isInCombat()) GetCombatOrders(); 
...
bool PlayerbotAI::TradeItem(const Item& item, int8 slot)
{
    ...
    {
        for( uint8 i=0; i<(uint8) TRADE_SLOT_TRADED_COUNT; ++i )
        {
		if( m_bot->GetTradeData()->GetItemByTradeSlot(i)==NULL ) {
                tradeSlot = i;
                break;
            }
        }
...
void PlayerbotAI::HandleCommand(const std::string &text, Player &fromPlayer)
{
	...
	// handle 'follow' or 'come' commands
    else if(text == "follow" || text == "come")
        Follow(*m_master);

	// handle 'stay' or 'stop' commands
    ...
    // handle 'poison' mainhand weapon command
    else if(text.size() > 2 && text.substr(0, 2) == "p " || text.size() >= 8 && text.substr(0, 8) == "poison m")
    {
    ...
    // handle 'poison' offhand weapon command
    ...
    else if(text == "spells")
    {
        ...
        SendWhisper("Here are my non-attack spells:", fromPlayer);
        ch.SendSysMessage(posOut.str().c_str());
        SendWhisper("Here are my attack spells:", fromPlayer);
        ch.SendSysMessage(negOut.str().c_str());
    }

	// handle 'accept quest' command
    else if (text.size() > 13 && text.substr(0, 13) == "accept quest ")
    {
        ...
        if (questId == 0)
            questId = getQuestId(questStr.c_str(), true);
		
		// if quest ID could not be obtained, tell master 
		if (questId == 0)
		{
			TellMaster("I can't figure out the quest reference. Shift-click it for me.");
			return;
		}
	...
	// handle 'quests' commands
    else if (text.size() == 1 && text.substr(0,1) == "q" || text == "quests")
    {
        ...
        for (QuestStatusMap::iterator iter=m_bot->getQuestStatusMap().begin(); iter!=m_bot->getQuestStatusMap().end(); ++iter)
        {
            ...
            const uint32 questId = qInfo->GetQuestId();
			const uint32 questLvl = qInfo->GetQuestLevel();
            const std::string name = qInfo->GetTitle();
            if (name.empty())
                continue;
            //out << qData->m_status << " ";
            ...
            {
                msg << "Quest " << " |cffffffff|Hquest:" << questId << ":" << questLvl << "|h[" << name << "]|h|r" << " ";
                msg << " complete" << "\n";
            }
    ...
    {
        Unit *const unit = m_master->GetSelectedUnit();
        ...
        if (!unit->isVendor() && !unit->isInnkeeper())
        {
            TellMaster("This is not a trader... I can't sell my stuff to this person.");
            return;
        }
        ...
        if (TotalSold > 0) {
            report << "Sold a total of " << TotalSold << " item(s) for ";
	...
	else
    {
        ...
        if (!itemIds.empty()) 
		{
            ...
            for (uint32 iI = 0; !wasRewarded && iI < questMenu.MenuItemCount(); ++iI)
            {
                ...
                if (status == QUEST_STATUS_COMPLETE && ! m_bot->GetQuestRewardStatus(questID) && pQuest->GetRewChoiceItemsCount() > 1 
...
bool PlayerbotAI::DoLoot()
{
    ...
    if(!m_lootCurrent)
    {
        ...
        if(m_bot->IsWithinDistInMap(object, INTERACTION_DISTANCE))
        {
			...
			//m_bot->GetPlayerbotAI()->TellMaster("I am looting "+(std::string)object->GetName()+".");
