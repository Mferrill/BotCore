...
PlayerbotClassAI::PlayerbotClassAI(Player *master, Player *bot, PlayerbotAI *ai): m_master(master), m_bot(bot), m_ai(ai), rezSpamTimer(0)
{
...
Unit *PlayerbotClassAI::FindMainTankInRaid(Player *gPlayer)
{
    ...
    {
        QueryResult_AutoPtr result = CharacterDatabase.PQuery("SELECT memberGuid FROM group_member LEFT JOIN groups ON group_member.guid=groups.guid WHERE memberFlags='%u' AND leaderGuid = '%u'", MEMBER_FLAG_MAINTANK, pGroup->GetLeaderGUID());
        if(result)
        {    
...
Unit *PlayerbotClassAI::FindMainAssistInRaid(Player *gPlayer)
{
    ...
    if (pGroup->isRaidGroup())
    {
        QueryResult_AutoPtr result = CharacterDatabase.PQuery("SELECT memberGuid FROM group_member LEFT JOIN groups ON group_member.guid=groups.guid WHERE memberFlags='%u' AND leaderGuid = '%u'", MEMBER_FLAG_MAINASSIST, pGroup->GetLeaderGUID());
          if(result)
        {    
            uint64 pGuid = MAKE_NEW_GUID(result->Fetch()->GetInt32(),0,HIGHGUID_PLAYER);
            pPlayer = objmgr.GetPlayer(pGuid);
