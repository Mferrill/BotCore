...
bool Group::Create(const uint64 &guid, const char * name)
{
    ...
    m_lootMethod = m_lootMethod = (LootMethod)sConfig.GetIntDefault("Bot.LootMethod", 2);		// PlayerBot mod
    m_lootThreshold = ITEM_QUALITY_UNCOMMON;
    m_looterGuid = guid;
...
bool Group::_setMainTank(const uint64 &guid, const bool &apply)
{
	...
	if (pPlayer && pPlayer->IsPlayerbot()) 
	{
		if (apply) 
		{
			pPlayer->HandleEmoteCommand(EMOTE_ONESHOT_ROAR);		// if PlayerBot is maintank, acknowledge it
		} else {
	...
	if (apply)
		for (GroupReference *itr = GetFirstMember(); itr != NULL; itr = itr->next())
		{
			...
			if (tPlayer && tPlayer->IsPlayerbot()) 
				{
				PlayerbotAI *ai = tPlayer->GetPlayerbotAI();
				if (ai)	
					tPlayer->GetPlayerbotAI()->GetClassAI()->SetMainTank(objmgr.GetPlayer(guid));
			}
		}
