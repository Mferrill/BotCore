...
void AuctionHouseBot::isItemAvailableForAuction(uint32 itemToCheckID) {

	bool debug_Out = true;
    bool debug_Out_Filters = true;
    ...
    if (AHBSeller) {
		...
		this->itemAuctionsResult="AH Bot is currently disabled";
		return; 
	}
    ...
    QueryResult_AutoPtr results = QueryResult_AutoPtr(NULL);
    char npcQuery[] = "SELECT distinct item FROM npc_vendor";
	if(npcItems.size() >0 ) { //should already be filled
	} else {
		sLog.outString("AuctionHouseBot.isItemAvailableForAuction: npcItems start size=%i",npcItems.size()); 
		...
		sLog.outString("AuctionHouseBot.isItemAvailableForAuction: lootItems end size=%i ",npcItems.size());
	}

	if(lootItems.size() >0 ) { //should already be filled
	} else {
		sLog.outString("AuctionHouseBot.isItemAvailableForAuction: lootItems start size=%i",lootItems.size()); 
		char lootQuery[] = "SELECT item FROM creature_loot_template UNION "
			"SELECT item FROM reference_loot_template UNION "
			"SELECT item FROM disenchant_loot_template UNION "
			"SELECT item FROM fishing_loot_template UNION "
		...
		sLog.outString("AuctionHouseBot.isItemAvailableForAuction: lootItems end size=%i ",lootItems.size());
	}
		
	uint32 itemToCheck=itemToCheckID;

    ItemPrototype const* prototype = objmgr.GetItemPrototype(itemToCheckID);

	if (prototype == NULL) {
		this->itemAuctionsResult="Item was not found";
        return; // "Item was not found";
	}
	if (debug_Out) {
		sLog.outString("AuctionHouseBot: Processing ItemID=%i %s Quality=%i Item Class=%i",prototype->ItemId,prototype->Name1,prototype->Quality,prototype->Class);
	}
	switch (prototype->Bonding) {
		case NO_BIND:
			if (!No_Bind) {
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Bind Type settings in AH (No Bind)";
				return;
			}
			break;
		case BIND_WHEN_PICKED_UP:
			if (!Bind_When_Picked_Up) {
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Bind Type settings in AH (BOPickup)";
				return;
			}
			break;
		case BIND_WHEN_EQUIPED:
			if (!Bind_When_Equipped) {
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Bind Type settings in AH (BOEquip)";
				return;
			}
			break;
		case BIND_WHEN_USE:
			if (!Bind_When_Use) {
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Bind Type settings in AH (Bind When Use)";
				return;
			}
			break;
		case BIND_QUEST_ITEM:
			if (!Bind_Quest_Item) {
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Bind Type settings in AH (Bind Quest Item)";
				return;
			}
			break;
		default:
			this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Bind Type settings in AH (Bind Default) Prolly should never get here.  Problem with Config File";
			return;
			break;
    }
	if (debug_Out) {
		sLog.outString("AuctionHouseBot: Processing %s cleared bonding",prototype->Name1);
	}

	switch (SellMethod) {
		case 0:
			if (prototype->SellPrice == 0) {
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Sell Method settings in AH (sellprice=0)";
				return; 
			}
			break;
		case 1:
			if (prototype->BuyPrice == 0) {
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Sell Method settings in AH (buyprice=0)";
				return; 
			}
        break;
    }
	...
	if (debug_Out) {
		sLog.outString("AuctionHouseBot: Processing %s cleared sell method",prototype->Name1);
	}

	if ((prototype->Quality < 0) || (prototype->Quality > 6)) {
		snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c of quality settings in AH (quality<0 or >6) quality=%i",prototype->Quality);
		return;
	}
	if (debug_Out) {
		sLog.outString("AuctionHouseBot: Processing %s cleared quality",prototype->Name1);
	}

	bool vendorHasItem=false;
    for (unsigned int i = 0; (i < npcItems.size()) && (!vendorHasItem); i++)
    {
		if (itemToCheck == npcItems[i]) {
			//sLog.outString("i=%i Item to Check=%i  npcItem=%i",i,itemToCheck,npcItems[i]);
			vendorHasItem = true;
		}
    }
    ...
    if ((Vendor_Items == 0) && !(prototype->Class == ITEM_CLASS_TRADE_GOODS)) {
        bool isVendorItem = vendorHasItem;
		if (isVendorItem) {
			this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Vendor settings in AH (Vendor Item)";
			return; 
		}
    }

	if (debug_Out) {
		sLog.outString("AuctionHouseBot: %s cleared Vendor Items",prototype->Name1);
	}

	if ((Vendor_TGs == 0) && (prototype->Class == ITEM_CLASS_TRADE_GOODS)) {
		bool isVendorTG = vendorHasItem;
		if (isVendorTG) {
			this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Vendor settings in AH (Vendor Trade Goods)";
			return; 
		}
    }
	if (debug_Out) {
		sLog.outString("AuctionHouseBot: %s cleared Vendor TGs",prototype->Name1);
	}
	bool itemIsLoot=false;
	for (unsigned int i = 0; (i < lootItems.size()) && (!itemIsLoot); i++) {
		if (itemToCheck == lootItems[i])
			itemIsLoot = true;
	}

	if ((Loot_Items == 0) && !(prototype->Class == ITEM_CLASS_TRADE_GOODS)) {
        bool isLootItem = itemIsLoot;
		if (isLootItem) {
			this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Loot Items settings in AH (Loot Item)";
			return; 
		}
    }
	if (debug_Out) {
		sLog.outString("AuctionHouseBot: %s cleared Loot Items",prototype->Name1);
	}

	if ((Loot_TGs == 0) && (prototype->Class == ITEM_CLASS_TRADE_GOODS)) {
		bool isLootTG = itemIsLoot;
		if (isLootTG) {
			this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Loot settings in AH (Loot Trade Good Item)";
			return; 
		}
    }

	if (debug_Out) {
		sLog.outString("AuctionHouseBot: %s cleared Loot TGs",prototype->Name1);
	}
	...
	if ((Other_Items == 0) && !(prototype->Class == ITEM_CLASS_TRADE_GOODS)) {
		...
		if ((!isLootItem) && (!isVendorItem)) {
			this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Other Items settings in AH";
			return; 
		}
    }
	if (debug_Out) {
		sLog.outString("AuctionHouseBot: %s cleared other items",prototype->Name1);
	}
	...
	if ((Other_TGs == 0) && (prototype->Class == ITEM_CLASS_TRADE_GOODS)) {
                ...
                bool isLootTG = itemIsLoot;
		if ((!isLootTG) && (!isVendorTG)) {
			this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Other Trade Goods settings in AH";
			return; 
		}
    }
	if (debug_Out) {
		sLog.outString("AuctionHouseBot: %s cleared Other Trade Goods",prototype->Name1);
	}
            ...
            if ((DisableBeta_PTR_Unused) && ((prototype->ItemId == 21878) || (prototype->ItemId == 27774) || (prototype->ItemId == 27811) || (prototype->ItemId == 28117) || (prototype->ItemId == 28112))) {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (PTR/Beta/Unused Item)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c it is a PTR/Beta/Unused Item";
				return;
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Permanent Enchant Item)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Disable Permanent Enchant Items setting in AH";
				return;
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Conjured Consumable)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Disable Conjured Items setting in AH";
				return;
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Gem)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Disable Gems setting in AH";
				return;
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Money)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Disable Money Items setting in AH";
				return;
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Lootable Item)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Disable Lootable Items setting in AH";
				return;
            }
            ...
            if ((DisableKeys) && (prototype->Class == ITEM_CLASS_KEY))
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Keys)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Disable Keys setting in AH";
				return;
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Has a Duration)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Disable Items with a Duration setting in AH";
				return;
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (BOP or BQI and Required Level is less than Item Level)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c of Disabled Bind On Pickup or Bind Quest Item and Required Level is less than Item Level setting in AH";
				return;
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Warrior Item)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c Warrior Items are disabled setting in AH";
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Paladin Item)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c Paladin Items are disabled in AH Bot";
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Hunter Item)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c Hunter Items are disabled in AH Bot";
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Rogue Item)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c Rogue Items are disabled in AH Bot";
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Priest Item)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c Priest Items are disabled in AH Bot";
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (DK Item)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c Death Knight Items are disabled in AH Bot";
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Shaman Item)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c Shaman Items are disabled in AH Bot";
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Mage Item)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c Mage Items are disabled in AH Bot";
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Warlock Item)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c Warlock Items are disabled in AH Bot";
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Unused Item)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c Unused Class Items are disabled in AH Bot";
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Druid Item)", prototype->ItemId);
				this->itemAuctionsResult="Item is NOT sold by AH Bot b/c Druid Items are disabled in AH Bot";
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Item Level = %u)", prototype->ItemId, prototype->ItemLevel);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c Items below level %u are disabled setting in AH (Item Level=%u)",DisableItemsBelowLevel,prototype->ItemLevel);
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Item Level = %u)", prototype->ItemId, prototype->ItemLevel);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c Items above level %u are disabled setting in AH (Item Level=%u)",DisableItemsAboveLevel,prototype->ItemLevel);
				return; 
            }
           ...
           // Disable Trade Goods below level X
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Trade Good %u disabled (Trade Good Level = %u)", prototype->ItemId, prototype->ItemLevel);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c Trade Goods below level %u are disabled setting in AH (Item Level=%u)",DisableTGsBelowLevel,prototype->ItemLevel);
				return; 
            }
           ...
           // Disable Trade Goods above level X
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Trade Good %u disabled (Trade Good Level = %u)", prototype->ItemId, prototype->ItemLevel);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c Trade Goods above level %u are disabled setting in AH (Item Level=%u)",DisableTGsAboveLevel,prototype->ItemLevel);
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Item Level = %u)", prototype->ItemId, prototype->ItemLevel);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c Items below GUID %u are disabled setting in AH (Item Level=%u)",DisableItemsBelowGUID,prototype->ItemLevel);
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Item Level = %u)", prototype->ItemId, prototype->ItemLevel);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c Items above GUID %u are disabled setting in AH (Item Level=%u)",DisableItemsAboveGUID,prototype->ItemLevel);
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Trade Good Level = %u)", prototype->ItemId, prototype->ItemLevel);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c Trade Goods below GUID %u are disabled setting in AH (Item Level=%u)",DisableTGsBelowGUID,prototype->ItemLevel);
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (Trade Good Level = %u)", prototype->ItemId, prototype->ItemLevel);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c Trade Goods above GUID %u are disabled setting in AH (Item Level=%u)",DisableTGsAboveGUID,prototype->ItemLevel);
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (RequiredLevel = %u)", prototype->ItemId, prototype->RequiredLevel);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c Items below Level %u are disabled setting in AH (Item Level=%u)",DisableItemsBelowReqLevel,prototype->ItemLevel);
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (RequiredLevel = %u)", prototype->ItemId, prototype->RequiredLevel);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c Items above Level %u are disabled setting in AH (Item Level=%u)",DisableItemsAboveReqLevel,prototype->ItemLevel);
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Trade Good %u disabled (RequiredLevel = %u)", prototype->ItemId, prototype->RequiredLevel);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c Trade Goods below Required Level %u are disabled setting in AH (Item Level=%u)",DisableTGsBelowReqLevel,prototype->ItemLevel);
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Trade Good %u disabled (RequiredLevel = %u)", prototype->ItemId, prototype->RequiredLevel);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c Trade Goods above Required Level %u are disabled setting in AH (Item Level=%u)",DisableTGsAboveReqLevel,prototype->ItemLevel);
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (RequiredSkillRank = %u)", prototype->ItemId, prototype->RequiredSkillRank);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c items below Required Skill Rank %u are disabled setting in AH (Item Level=%u)",DisableItemsBelowReqSkillRank,prototype->ItemLevel);
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (RequiredSkillRank = %u)", prototype->ItemId, prototype->RequiredSkillRank);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c items above Required Skill Rank %u are disabled setting in AH (Item Level=%u)",DisableItemsAboveReqSkillRank,prototype->ItemLevel);
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (RequiredSkillRank = %u)", prototype->ItemId, prototype->RequiredSkillRank);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c Trade Goods below Required Skill Rank %u are disabled setting in AH (Item Level=%u)",DisableTGsBelowReqSkillRank,prototype->ItemLevel);
				return; 
            }
            ...
            {
                if (debug_Out_Filters) sLog.outString("AuctionHouseBot: Item %u disabled (RequiredSkillRank = %u)", prototype->ItemId, prototype->RequiredSkillRank);
				snprintf(this->itemAuctionsResult,150,"Item is NOT sold by AH Bot b/c Trade Goods above Required Skill Rank %u are disabled setting in AH (Item Level=%u)",DisableTGsAboveReqSkillRank,prototype->ItemLevel);
				return; 
            }

			if (debug_Out) {
				sLog.outString("AuctionHouseBot: Processing %s Item is available for AH. Quality=%i Item Class=%i",prototype->Name1,prototype->Quality,prototype->Class);
			}
			this->itemAuctionsResult="Item is available for AH Bot to randomly list for sale in the Auction House";
			return;
}
