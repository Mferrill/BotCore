...
#include "../../Bots/PlayerbotAI.h"
#include "../../Bots/PlayerbotClassAI.h"
...
Item* TradeData::GetItemByTradeSlot(uint8 slot) const
{
	return m_items[slot] ? m_player->GetItemByGuid(m_items[slot]) : NULL;
}
...
void Player::Update(uint32 p_time)
{
	...
	// PlayerBot mod
    if(IsPlayerbot() && m_playerbotAI) m_playerbotAI->UpdateAI(p_time);
}
...
void Player::SendTeleportPacket(Position &oldPos)
{
    ...
    if (!IsPlayerbot()) Relocate(&oldPos);		// PlayerBot mod
...
// PlayerBot mod
Player *Player::GetObjPlayer(uint64 guid) const
{
...
void Player::SetPlayerbotAI(PlayerbotAI *ai)
{
	...
	if(m_playerbotAI)
    {
        sLog.outError("Tried to reassign playerbot AI; this is not supported!");
        return;
    }
...
Player::GetNPCIfCanInteractWith(uint64 guid, uint32 npcflagmask)
{
    ...
    // not too far
    if (!unit->IsWithinDistInMap(this,INTERACTION_DISTANCE) && !IsPlayerbot())		// PlayerBot mod
        return NULL;
...
void Player::RemoveFromGroup(Group* group, uint64 guid)
{
    if (group)
    {
        ...
        if (_player) 
		{
			...
			//PlayerBot mod: if a player removes himself from a group, log out all of his playerbots
			//save the map of playerbots first because if the map gets altered when
			//a playerbot logs out, it will corrupt the for loop
...
Item* Player::GetItemFromBuyBackSlot(uint32 slot) const
{
...
void Player::KilledMonsterCredit(uint32 entry, uint64 guid)
{
	...
	// PlayerBot mod
    if(IsPlayerbot() && m_playerbotAI) m_playerbotAI->KilledMonster(entry, guid);
}
            ...
            if (qInfo->HasFlag(QUEST_TRINITY_FLAGS_KILL_OR_CAST))
            {
                for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
                {
                    ...
                    if (curCastCount < reqCastCount)
                    {
						...
						// PlayerBot mod
						if (IsPlayerbot()) GetPlayerbotAI()->TellMaster("Talked to quest guy.");
                    }
