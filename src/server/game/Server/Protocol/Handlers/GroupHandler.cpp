...
void WorldSession::HandleGroupUninviteGuidOpcode(WorldPacket & recv_data)
{
    ...
    if (grp->IsMember(guid))
    {
		...
		// PlayerBot mod: log out if a PlayerBot
		Player *botPlayer = objmgr.GetPlayer(guid);
...
void WorldSession::HandleGroupUninviteOpcode(WorldPacket & recv_data)
{
    ...
    PartyResult res = player->CanUninviteFromGroup();
    if (res != ERR_PARTY_RESULT_OK)
    {
        SendPartyResult(PARTY_OP_UNINVITE, "", res);
        return;
    ...
    {
        Player::RemoveFromGroup(grp,guid);
		// PlayerBot mod: check if PlayerBot and log out
		Player *botPlayer = objmgr.GetPlayer(guid);
		if(botPlayer && botPlayer->IsPlayerbot())
			player->GetSession()->LogoutPlayerBot(guid, true);
        return;
    }
