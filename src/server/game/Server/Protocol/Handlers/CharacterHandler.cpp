...
class CharacterHandler
{
    ...
    public:
        ...
        void HandlePlayerBotLoginCallback(QueryResult_AutoPtr /*dummy*/, SqlQueryHolder *holder)
        {
			...
			if(!bot) // Probably excessively verbose..
            {
			...
			if (master->IsPlayerbot())
				sLog.outError("WARNING: Adding PlayerBot to a non-player!");
			else
				sLog.outBasic("Adding PlayerBot '%s' for Master '%s'.", bot->GetName(), master->GetName());

            //Tell the master session it now manages this new bot
            (masterSession->m_playerBots)[guid] = bot;

			//Perform necessary checks: 
			// 1. if group doesn't exist, create a new one, etc.
			// 2. if group is full, remove bot from world and notify the master			
			if (!group)		
			{
				group = new Group;
				group->Create(master->GetGUID(), master->GetName());
				objmgr.AddGroup(group);
				group->AddMember(bot->GetGUID(), bot->GetName());	
			}
			else if (group->IsFull()) 
			{
				masterSession->LogoutPlayerBot(bot->GetGUID(), false);
				ChatHandler chH = ChatHandler(master);
			...
			// Add PlayerBot to map
			//bot->GetMap()->Add(bot);

			//Give the bot some AI; object is owned by the Player class
            PlayerbotAI *ai = new PlayerbotAI(master, bot);
			...
			bot->Say((std::string)bot->GetName()+" , at your service.", LANG_UNIVERSAL);
