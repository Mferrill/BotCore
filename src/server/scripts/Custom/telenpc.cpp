...
bool GossipHello_pryds_telenpc(Player *player, Creature * pCreature)
{
 ...
 //Config sConfig;
  ...
  if(sConfig.GetBoolDefault("pryds_pryds_telenpc.OnlyGMs", false)) // If pryds_pryds_telenpc.OnlyGMs is enabled in trinitycore.conf
    ...
    bool EnableFactionMalls = sConfig.GetBoolDefault("pryds_pryds_telenpc.EnableFactionMalls", true);
    bool EnableShoppingMall = sConfig.GetBoolDefault("pryds_pryds_telenpc.EnableShoppingMall", true);
...
bool EnableResSickness = TScriptConfig.GetBoolDefault("TeleNPC.EnableRemoveResSickness", false);
...
switch(action)
{
	...
	case 60:
		...
		{
		    player->PlayerTalkClass->GetGossipMenu().AddMenuItem(8, REQUESTER_MENU_REQUEST_ITEM, ITEM_REQUESTER_MENU_SELECT_ITEM, GOSSIP_ACTION_INFO_DEF + 1, REQUESTER_MENU_REQUEST_CONFIRMATION, 0, true);
...
case 1098:  // Teleport Category Menu
    ...
    player->ADD_GOSSIP_ITEM( 1, "PryDevCore Faction Mall"                  , GOSSIP_SENDER_MAIN, 2002);
    player->ADD_GOSSIP_ITEM( 4, "PryDevCore FFA Mall"                       , GOSSIP_SENDER_MAIN, 2003);
...
case 2407://Remove Res Sickness
	if (!player->HasAura(15007,0))
	{
		...
		GossipHello_pryds_telenpc(player, pCreature);
		return;
	}
...
bool GossipSelect_pryds_telenpc(Player *player, Creature * pCreature, uint32 sender, uint32 action)
{
  // Main menu
...
bool OnGossipSelectWithCode_pryds_telenpc(Player *player, Creature *pCreature, uint32 sender, uint32 action, const char *pCode)
		{
			switch(sender)
			{
				case ITEM_REQUESTER_MENU_SELECT_ITEM:
				{
					...
					if(itemId == 0)
					{
						...
						if(QueryResult_AutoPtr queryResult = WorldDatabase.PQuery("SELECT `entry` FROM `item_template` WHERE `name` = '%s'", escapedCode.c_str()))
							itemId = queryResult->Fetch()[0].GetUInt32();
					}

					const ItemPrototype *itemProto = ObjectMgr::GetItemPrototype(itemId);
					if(itemProto == NULL)
					{
		...
		bool CreateItemAndMailToPlayer_pryds_telenpc(Player *player, uint32 itemId)
		{
			Item *pItem = new Item();
			if(pItem->Create(objmgr.GenerateLowGuid(HIGHGUID_ITEM), itemId, player) == false)
				return false;
			...
			mailDraft.SendMailTo(MailReceiver(player), toSend);
...
void AddSC_pryds_telenpc()
{
  ...
  newscript->Name="pryds_telenpc";
  newscript->pGossipHello = &GossipHello_pryds_telenpc;
  newscript->pGossipSelect = &GossipSelect_pryds_telenpc;
