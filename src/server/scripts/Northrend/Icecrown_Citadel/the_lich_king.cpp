...
struct boss_lich_kingAI : public ScriptedAI
{
    ...
    void Reset()
    {
		...
		/******************* Phasen  ******************/
		m_uiPhase = 0;
		m_uiRandomSpeechTimer = urand(25*IN_MILLISECONDS,35*IN_MILLISECONDS);
		m_uiBerserkTimer =	15*MINUTE*IN_MILLISECONDS;

		/******************* Spells Phase 1 ********************/
		m_uiSummonShamblingHorrorTimer	= 20*IN_MILLISECONDS;
		m_uiSummonDrudgeGhoulsTimer		= 10*IN_MILLISECONDS;
		m_uiInfestTimer					= 30*IN_MILLISECONDS;
		m_uiNecroticPlagueTimer			= urand(29*IN_MILLISECONDS,30*IN_MILLISECONDS);
		m_uiPlagueSiphonTimer			= 5*IN_MILLISECONDS;
		m_uiTirionSpawneTimer			= 5*IN_MILLISECONDS;

		/****************** Spells Phase 1.5 & Phase 2.5 **************/
		m_uiRemorselesWinterTimer = 5*IN_MILLISECONDS;
		m_uiQuakeTimer = 70*IN_MILLISECONDS;
		m_uiIcePulsSummonTimer	= 10*IN_MILLISECONDS;
		m_uiPainandSufferingTimer = urand(10*IN_MILLISECONDS,11*IN_MILLISECONDS);
		m_uiSummonSpiritTimer = 18*IN_MILLISECONDS;

		/******************* Spells Phase 2 *********************/
		m_uiSummonValkyrTimer	= 20*IN_MILLISECONDS;
		m_uiSoulReaperTimer		= 15*IN_MILLISECONDS;
		m_uiDefileTimer			= 25*IN_MILLISECONDS;
		m_uiInfestTimer			= 40*IN_MILLISECONDS;

		/******************* Spells Phase 3 **********************/
		m_uiSummonVileSpiritTimer = 30*IN_MILLISECONDS;
		m_uiHarvestSoulTimer = 70*IN_MILLISECONDS;
		m_uiFrostGramPortTimer = 900*IN_MILLISECONDS;
       ...
       if(!UpdateVictim())
		...
		if(!m_bIsDeathPhase)
		{
			if (m_uiRandomSpeechTimer < uiDiff)
			{
			   ...
			   m_uiRandomSpeechTimer = urand(30*IN_MILLISECONDS,35*IN_MILLISECONDS);
			}
		...
		if(m_uiPhase == 1)
		{
			if (m_uiInfestTimer < uiDiff)
            {
                ...
                m_uiInfestTimer = urand(30*IN_MILLISECONDS,35*IN_MILLISECONDS);
            }
			...
			if (m_uiPlagueSiphonTimer < uiDiff)
            {
                ...
                m_uiPlagueSiphonTimer = urand(29*IN_MILLISECONDS,30*IN_MILLISECONDS);
            }
			...
			if (m_uiSummonDrudgeGhoulsTimer < uiDiff)
            {
                ...
                m_uiSummonDrudgeGhoulsTimer = urand(20*IN_MILLISECONDS,21*IN_MILLISECONDS);
            }
			...
			if (m_uiSummonShamblingHorrorTimer < uiDiff)
            {
                ...
                m_uiSummonShamblingHorrorTimer = urand(60*IN_MILLISECONDS,61*IN_MILLISECONDS);
            }
			...
			if (m_uiNecroticPlagueTimer < uiDiff)
            {
                ...
                m_uiNecroticPlagueTimer = urand(5*IN_MILLISECONDS,10*IN_MILLISECONDS);
            }
		...
		if(m_uiPhase == 2)
		{
			if (m_uiRemorselesWinterTimer < uiDiff)
            {
                ...
                m_uiRemorselesWinterTimer = 90*IN_MILLISECONDS;
            }
			...
			if (m_uiSummonSpiritTimer < uiDiff)
            {
				...
				m_uiSummonSpiritTimer	= 16*IN_MILLISECONDS;
            }
			...
			if (m_uiIcePulsSummonTimer < uiDiff)
            {
				...
				m_uiIcePulsSummonTimer	= 15*IN_MILLISECONDS;
            }
			...
			if (m_uiPainandSufferingTimer < uiDiff)
            {
				...
				m_uiPainandSufferingTimer = urand(1.5*IN_MILLISECONDS,2.0*IN_MILLISECONDS);
            }
		...
		if(m_uiPhase == 3)
		{
			if (m_uiDefileTimer < uiDiff)
            {
                ...
                m_uiDefileTimer = 20*IN_MILLISECONDS;
            }
			...
			if (m_uiSoulReaperTimer < uiDiff)
            {
                ...
                m_uiSoulReaperTimer = 20*IN_MILLISECONDS;
            }
		...
		if(m_uiPhase == 4)
		{
			...
			if (m_uiPainandSufferingTimer < uiDiff)
            {
                if (Unit* target = SelectUnit(SELECT_TARGET_RANDOM,0))
                DoCast(target, SPELL_PAIN_AND_SUFFERING);
				m_uiPainandSufferingTimer = urand(2*IN_MILLISECONDS,4*IN_MILLISECONDS);
            }
            else m_uiPainandSufferingTimer -= uiDiff;
...
pFather->SetReactState(REACT_PASSIVE);//не пашет
...
struct mob_IcePulsAI : public ScriptedAI
{
	...
	void Reset()
    {
		m_uiIcePulseTimer	= 2*IN_MILLISECONDS;
		m_uiIceBurstCheckTimer	= 2*IN_MILLISECONDS;
    }
...
struct mob_ValkyrAI : public ScriptedAI
{
    ...
    {
		me->SetFlying(true);
		m_uiGrabTimer = 2*IN_MILLISECONDS;
		m_uiMovementTimer = 3*IN_MILLISECONDS;
		m_uiFallPlayerTimer = 10*IN_MILLISECONDS;
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        if (!UpdateVictim()) 
		 ...
		 if (m_uiGrabTimer < uiDiff)
         {
            ...
            m_uiGrabTimer = 120*IN_MILLISECONDS;
         } 
		...
		if (m_uiMovementTimer < uiDiff)
        {
			...
			m_uiMovementTimer = 120*IN_MILLISECONDS;
        } 
		...
		if (m_uiFallPlayerTimer < uiDiff)
        {
			...
			m_uiFallPlayerTimer = 120*IN_MILLISECONDS;;
        } 
