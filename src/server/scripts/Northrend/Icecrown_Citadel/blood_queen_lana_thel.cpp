...
struct Boss_Blood_Queen_LanathelAI : public ScriptedAI
{
	...
	void Reset()
	{
		...
		/************ Phase 1 **********/ 
		m_uiTwilightBloodboltTimer = 10*IN_MILLISECONDS;
		m_uiVampBiteTimer	= 15*IN_MILLISECONDS;
		m_uiSwarmingShadowsTimer = 30*IN_MILLISECONDS;
		m_uiPactofDarkfallenTimer	= 5*IN_MILLISECONDS;
		/************ Phase 2 **********/ 
		m_uiSetHoverTimer = 900*IN_MILLISECONDS;
		m_uiBloodboldSplashTimer = 900*IN_MILLISECONDS;
		m_uiLandingTimer = 900*IN_MILLISECONDS;;
		m_uiFlyingFalseTimer = 900*IN_MILLISECONDS;;
...
//					PactFallen();
					...
					m_uiPactofDarkfallenExplodeTimer = 2*IN_MILLISECONDS;
					m_uiPactofDarkfallenTimer = 25*IN_MILLISECONDS;
				} 
				...
				if (m_uiTwilightBloodboltTimer < uiDiff)
				{
					...
					m_uiTwilightBloodboltTimer  = urand(6*IN_MILLISECONDS,12*IN_MILLISECONDS);
				}
				...
				if (m_uiVampBiteTimer < uiDiff)
				{
					...
					m_uiVampBiteTimer	= urand(15*IN_MILLISECONDS,60*IN_MILLISECONDS);
				}
				...
				{	
					...
					m_uiSetHoverTimer = 5*IN_MILLISECONDS;
					m_uiBloodboldSplashTimer = 10*IN_MILLISECONDS;
					m_uiLandingTimer = 26*IN_MILLISECONDS;
					m_uiFlyingFalseTimer = 30*IN_MILLISECONDS;
				}
		...
		if(m_uiPhase == 2)
		{
			if (m_uiSetHoverTimer < uiDiff)
			{
				...
				me->SetFlying(true);
				m_uiSetHoverTimer	= 90*IN_MILLISECONDS;
			}
			...
			if (m_uiBloodboldSplashTimer < uiDiff)
			{
               std::list<Unit*> pTargets;
				...
				m_uiBloodboldSplashTimer = 2*IN_MILLISECONDS;
			}
